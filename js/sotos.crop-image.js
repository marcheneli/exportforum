            ...
            //get the crop image and replace the edit canvas and output the Scope imageOut
              this.getImage =function () {
                 if($scope.cropOptions.viewShowCropTool){
                     ...
                     //if this set then the image out size is from the original image $scope.cropOptions.outputImageSelfSizeCrop
                     if( theSelection.w*ratio_width <= 1000 && theSelection.h*ratio_width <= 1000){
...
//                    srcCanvasCtx.enableImageSmoothing = 4;
            ...
            // # 0.0.8 and resize window
              ...
              if( $scope.cropOptions.outputImageWidth===0 || $scope.cropOptions.outputImageHeight===0){
                ...
                if($scope.cropOptions.outputImageRatioFixed
                    && $scope.cropOptions.outputImageWidth!=undefined
                    && $scope.cropOptions.outputImageHeight!=undefined
                ) {
                    var cropRatio = $scope.cropOptions.outputImageWidth/$scope.cropOptions.outputImageHeight;
                    if (editCanvas.height > editCanvas.width) {
                        theSelection.w = editCanvas.width * 0.95;
                        theSelection.h = theSelection.w / cropRatio;
                    }
                    if (editCanvas.height <= editCanvas.width)  {
                        theSelection.h = editCanvas.height * 0.95;
                        theSelection.w = theSelection.h * cropRatio;
                    }
                } else {
                    theSelection.h = editCanvas.height * 0.95;
                    theSelection.w = editCanvas.width * 0.95;
            ...
            var loadImage = function(){
                if($scope.cropOptions.image != undefined&&$scope.cropOptions.image!=null&$scope.cropOptions.image!='') {
          ...
          //  $scope.$watch('$scope.saveImage');
            ...
            $scope.$on('cropImageSave',function(){
                ...
                $scope.$emit("imageOut", $scope.imageOut);
          ...
          //       $scope.cropImageSave( )($scope.imageOut);
            ...
            this.rotateLeft = function(){
                theSelection.rotateCenter.angleRotate-=1.5708;
                self.drawScene();
            };
            ...
            $scope.$on('rotateLeft',self.rotateLeft);
            $scope.$on('rotateRight',self.rotateRight);
